...
/// Xtensa is excluded: its LLVM backend aborts with "Incomplete
/// scavenging after 2nd pass" when the debug machinery is compiled out,
/// so those builds keep it and pay the binary size.
const strip_debug_machinery = native_freestanding and
    builtin.cpu.arch != .xtensa;

const debug_machinery: bool = !strip_debug_machinery and
